 ...
 *
 * */
class Apptha_Qrcode_Model_Printstyle extends Mage_Core_Model_Abstract
{
 public function layoutCollection() {
...
//Function to update the QR Code which are already exist
	...
	{
		$count = array();
		$layoutCollection = Mage::getModel('qrcode/print')->getCollection();
		...
		foreach ($layoutCollection as $layout)
		{
			...
			$QRGetImage = $read->fetchRow("Select style_img  from " . $tprefix . "print where print_Id = " . $layoutId);
			if($QRGetImage[style_img])
			{
   		 	$image_path_qr = $PNG_DIR.$QRGetImage[style_img];
    	 	$get_Image = '<img src=' . $image_path_qr . ' width="400" height="200" >';
    		}
    		else
    		{
    	 	$get_Image ='Not available';
    		}
       $Count[$layoutId] = $get_Image;
      }
    return $Count;
	}
}?>